...
class ProductDetail extends Component {
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  ...
                  {/* Cập nhật sự kiện click cho nút DELETE */}
  ...
  // API Get Products đã được update logic phân trang
  apiGetProducts() {
    const config = { headers: { 'x-access-token': this.context.token } };
    axios.get('/api/admin/products?page=' + this.props.curPage, config).then((res) => {
      const result = res.data;
      // Nếu trang hiện tại còn sản phẩm thì hiển thị bình thường
      if (result.products.length !== 0) {
        this.props.updateProducts(result.products, result.noPages);
      } else {
        // Nếu trang hiện tại trống (do đã xóa hết), lùi về trang trước đó
        const nextPage = this.props.curPage - 1;
        if (nextPage > 0) {
           axios.get('/api/admin/products?page=' + nextPage, config).then((res) => {
            const result = res.data;
            this.props.updateProducts(result.products, result.noPages);
          });
        } else {
           // Trường hợp xóa hết sạch sản phẩm (về trang 0 thì giữ nguyên là rỗng)
           this.props.updateProducts(result.products, result.noPages);
        }
      }
    });
